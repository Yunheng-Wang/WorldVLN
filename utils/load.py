...
def load_checkpoint(self, accelerator, checkpoint_path, reset_scheduler):
    # 1. 查看文件的存在性
    if not os.path.exists(checkpoint_path):
        logger.warning(f"Checkpoint path {checkpoint_path} does not exist")
        return
    logger.info(f"Loading checkpoint from {checkpoint_path}")
    # 2. 通过保存文件提取 epoch 和 step
    step_match = re.search(r'step_(\d+)_(\d+)', checkpoint_path)
    epoch = int(step_match.group(1))
    global_step = int(step_match.group(2))
    ...
    # 4. 是否需要重置scheduler
    if reset_scheduler and config is not None and scheduler is not None:
        logger.info("Resetting scheduler to new configuration (not using checkpoint scheduler state)...")
        # 4.1 解由 accelerator 包装的调度器 
        unwrapped_scheduler = self.scheduler
        if hasattr(scheduler, 'module'):
            unwrapped_scheduler = scheduler.module
        # 4.2 检查是否是自定义调度器
