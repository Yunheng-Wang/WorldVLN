...
def load_task(config):
    grouped_tasks = {}
    with open(os.path.join(config["data_root"], "tasks", "r2r_ce", "train/train.json"), 'r', encoding='utf-8') as file:
        task = json.load(file)
...
def main():
    ...
    for i, scene_id in enumerate(tqdm(tasks, desc="Scenes", unit="scene")):
        ...
        for id, task in enumerate(tqdm(tasks[scene_id], desc="Tasks", unit="task", leave=False)):
            action = []
            ## 4.2 配置保存目录
            save_path = os.path.join(cfg["data_root"], "cache", "train", "mp3d_" + "r2r_ce" + "_" + scene_id + "_" + str(task["episode_id"]))
