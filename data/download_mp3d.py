...
def get_release_scans(release_file):
    with urllib.request.urlopen(release_file) as scan_lines:
        scans = [scan_line.rstrip(b'\n').decode('utf-8') for scan_line in scan_lines]
    return scans
...
def main():
    ...
    if args.task_data:
        if set(args.task_data) & set(TASK_FILES.keys()):  # download task data
            ...
            print('ERROR: Unrecognized task data id: ' + ','.join(args.task_data))
    ...
    if args.id and args.id != 'ALL':  # download single scan
        ...
        if len(file_types) == len(FILETYPES):
            print(f'WARNING: You are downloading the entire MP release which requires {RELEASE_SIZE} of space.')
        else:
            print(f'WARNING: You are downloading all MP scans of type {file_types[0]}')
